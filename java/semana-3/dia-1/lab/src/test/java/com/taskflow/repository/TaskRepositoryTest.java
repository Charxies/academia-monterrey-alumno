...
class TaskRepositoryTest {
    ...
    // ==================== TODO MP-8: @DataJpaTest A FONDO ====================
    //
    // (1) EL FALSO VERDE del caché de 1er nivel — demo + regla:
    //     persiste una tarea con em.persistFlushFind(...) y RELÉELA sin flush/clear -> verde.
    //     En clase el instructor rompe el mapeo (comenta un @Enumerated) y SIGUE verde: el objeto salió
    //     del caché de memoria, no de SQL. Añade em.clear() antes de releer con taskRepository.findById(id)
    //     -> ahora el find va a SQL de verdad. Regla: en tests de MAPEO, flush()+clear() antes de leer.
    //       @Test
    //       void mapeoDeStatus_sobreviveFlushYClear() throws TaskValidationException {
    //           Long pid = nuevoProyecto("Proyecto flush");
    //           Task p = em.persistFlushFind(new Task(null, "Mapeo honesto", "desc",
    //                   TaskStatus.IN_PROGRESS, Priority.HIGH, pid, 1L, null));
    //           em.clear();                                   // vacía el caché de 1er nivel
    //           Task releida = taskRepository.findById(p.getId()).orElseThrow();
    //           assertThat(releida.getStatus()).isEqualTo(TaskStatus.IN_PROGRESS);
    //       }
    //
    // (2) @Sql — escenario DECLARATIVO (el script ya está en src/test/resources/sql/escenario-tareas.sql):
    //     import org.springframework.test.context.jdbc.Sql;
    //       @Test
    //       @Sql("/sql/escenario-tareas.sql")               // 2 proyectos + 8 tareas, ids fijos
    //       void findByAssigneeIdAndStatus_sobreEscenarioSql_filtraCombinado() {
    //           List<Task> r = taskRepository.findByAssigneeIdAndStatus(10L, TaskStatus.TODO);
    //           assertThat(r).hasSize(2);                     // tareas 1 y 2 del script
    //       }
    //     Nota de juicio (dolor #12): los ids fijos del script valen SOLO en ESTE test (esquema fresco);
    //     jamás se asumen en otra clase.
    //
    // (3) Integrador (refactor c): findByStatus con @EnumSource(TaskStatus.class) — un parametrizado que
    //     cubre los 3 estados. import org.junit.jupiter.params.ParameterizedTest / provider.EnumSource.
    // ========================================================================
}
