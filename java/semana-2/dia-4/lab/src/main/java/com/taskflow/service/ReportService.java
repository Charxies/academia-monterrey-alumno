 ...
 *
 * ============================================================================================
 * TODO (S2D4, integrador) — REESCRIBIR PARCIALMENTE CON JUICIO. Cada método lleva UN comentario de
 * una línea justificando la decisión (el juicio ES el entregable). Cuando TaskRepository extienda
 * JpaRepository con las derived queries:
 *   1) buscarPorTitulo(q)      -> repo.findByTitleContainingIgnoreCase(q)   // DERIVED: el LIKE lo hace la BD.
 *   2) porcentajeCompletadas() -> repo.countByStatus(DONE) y repo.count()   // DERIVED: contar en la BD (* 100.0 vs la trampa long/long).
 *   3) pendientesPorFecha()    -> repo.findByStatusNot(DONE) + sorted(...)   // MIXTO: la BD filtra; el orden de los null es dominio.
 *   4) tareasPorEstado() y tareasPorAsignado() -> SIGUEN con findAll()+groupingBy // STREAMS: GROUP BY da conteos, no Map<K,List<Task>>.
 * findByAssigneeIdAndStatus queda declarada y testeada (la usa el stretch de D5); ningún camino
 * principal depende de ella todavía.
 * ============================================================================================
 */
@Service
