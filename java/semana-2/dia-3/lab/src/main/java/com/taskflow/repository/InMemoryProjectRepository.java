 ...
 *
 * TODO (integrador, paso 2): esta clase crece a CRUD completo —
 *   - EXTRAE la interfaz ProjectRepository (save/findById/findAll/deleteById) y decláralo aquí
 *     con 'implements ProjectRepository' (@Override en los 4 métodos).
 *   - EVOLUCIONA save a secuencia-upsert: como Project es inmutable (id primitivo, sin setId), si
 *     llega uno nuevo (id == 0) asigna el siguiente id RECONSTRUYENDO el Project; si trae id, avanza
 *     la secuencia al máximo (Math.max) y hace put. Añade un campo 'private long secuencia = 0;'.
 *   - AÑADE deleteById(Long id): void, store.remove(id).
 */
@Repository
