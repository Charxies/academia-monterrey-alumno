...
public class GlobalExceptionHandler {
    ...
    // TODO MP-5: añade DOS handlers nuevos (mismo patrón que los de arriba, usando construir(...)):
    //
    //   1) 409 CONFLICT — username duplicado al registrar:
    //        @ExceptionHandler(UsernameAlreadyExistsException.class)
    //        -> construir(HttpStatus.CONFLICT, ex.getMessage(), List.of());
    //
    //   2) 401 UNAUTHORIZED — credenciales inválidas en el login. El AuthenticationManager lanza
    //      org.springframework.security.authentication.BadCredentialsException DENTRO del controller
    //      /auth/login (endpoint público), así que SÍ llega a este advice (contraste con las
    //      excepciones del filtro JWT, que corren antes del DispatcherServlet y este advice NO ve):
    //        @ExceptionHandler(BadCredentialsException.class)
    //        -> construir(HttpStatus.UNAUTHORIZED, "Credenciales inválidas.", List.of());
